      ...
      <main className="max-w-7xl mx-auto px-6 py-10 space-y-10">
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Showing all registered users</p>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-white/5 font-mono">
                  {users.sort((a, b) => (b.lastLogon || '').localeCompare(a.lastLogon || '')).map((u) => {
                    const tradesPerLogon = u.totalLogons > 0 ? (u.totalTrades / u.totalLogons).toFixed(1) : '0.0'
                    return (
                      <tr key={u.id} className="hover:bg-white/[0.02] transition-colors group">
                        <td className="px-6 py-4">
                          <span className="text-sm font-bold text-white font-sans">{u.username}</span>
                        </td>
