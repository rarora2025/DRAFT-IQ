...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    for (const sport of sports) {
      ...
      const last1mWindowGames = Math.floor(lastUpdate / oneMin);
      
      // Always fetch games list if it's a new minute window, as scores change fast
      const shouldFetchGames = force || (current1mWindow > last1mWindowGames);
