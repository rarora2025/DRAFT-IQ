      ...
      try {
        ...
        supabase.auth.signInWithOAuth({
          ...
          if (error) {
            ...
            console.error('[Login] No URL returned from signInWithOAuth')
      ...
      >
        ...
        <form onSubmit={handleLogin} className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-6 backdrop-blur-sm">
          ...
          <button
            type="button"
            onClick={handleGoogleLogin}
            className="w-full h-14 bg-white/5 border border-white/10 hover:bg-white/10 text-white font-display font-bold text-lg rounded-xl flex items-center justify-center gap-3 transition-all active:scale-[0.98]"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24">
              <path
                fill="currentColor"
