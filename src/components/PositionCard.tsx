                      ...
                      >
                        ...
                        >
                              <Button
                                ...
                                className={`h-10 sm:h-11 w-full sm:w-auto px-8 sm:px-10 rounded-2xl ${isMarketLocked ? 'bg-gray-500/20 text-gray-500 cursor-not-allowed' : (isSellLocked ? 'bg-gray-500/40 text-gray-400 cursor-not-allowed' : 'bg-[#f8564e] hover:bg-[#e04a43] text-white shadow-lg shadow-red-500/20')} font-black uppercase text-xs`}
                              >
                                {checkingPrice ? <Loader2 className="w-4 h-4 animate-spin" /> : (isMarketLocked ? 'LOCKED' : (isSellLocked ? `${lockSecondsRemaining}S LOCK` : 'SELL'))}
                              </Button>
                        </motion.div>
                      )}
          ...
          <div className="bg-[#11122a] rounded-2xl px-2.5 sm:px-4 py-3 flex items-center justify-between border border-white/5 overflow-hidden">
            ...
            <div className="flex items-center gap-1 sm:gap-1.5 min-w-0">
              ...
              <div className="bg-[#11122a] rounded-2xl px-2.5 sm:px-4 py-3 flex items-center justify-between border border-white/5 overflow-hidden">
                ...
                <div className={`flex items-center gap-1 font-mono font-bold text-[11px] sm:text-sm shrink-0 ${isProfit ? 'text-emerald-400' : 'text-red-400'}`}>
