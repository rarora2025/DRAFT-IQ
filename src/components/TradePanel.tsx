...
export function TradePanel({ balance, currentTemp, onTrade, onPriceCheck, disabled, isDark = true, propType = 'Points', marketStatus, lastUpdated, isLiveGame, queuedTrades = [], onCancelQueuedTrade, defaultTolerance = 5, onUpdateDefaultTolerance }: TradePanelProps) {
  ...
  const canTrade = balance > 0 && !isLocked
  const canExecute = tradeSize > 0 && tradeSize <= balance
  ...
  const executeTrade = async () => {
    if (!pendingSide || !canExecute) return
          ...
          {status === 'price_changed' ? (
            ...
            >
              <div className="text-center space-y-4">
                ...
                <h3 className="text-3xl font-black text-white uppercase tracking-tighter">Set Position Size</h3>
              </div>
              ...
              <div className="space-y-6">
                  ...
                  <div className="flex items-baseline gap-1">
                    ...
                    {isEditingStake ? (
                      <input
                        ...
                        className="text-6xl font-black font-mono tracking-tighter text-white bg-transparent border-b-2 border-primary focus:outline-none w-48 text-center"
                        autoFocus
                      />
                    ) : (
                      <span 
                ...
                <div className="rounded-[2rem] p-6 space-y-4 bg-white/5 border border-white/10">
                  ...
                  <div className="border-t border-white/5 pt-4">
                    <div className="flex justify-between items-center">
                      <span className="font-black uppercase tracking-widest text-[10px] text-zinc-500">Trading Model</span>
                      <span className="text-[10px] font-black text-primary uppercase tracking-widest">Size × % Projection Drift</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 sm:gap-4">
                <motion.div whileHover={{ scale: canTrade ? 1.02 : 1 }} whileTap={{ scale: canTrade ? 0.98 : 1 }}>
                  <Button
                    ...
                    className={`w-full h-24 sm:h-32 ${isLocked ? 'bg-gray-500/20 text-gray-500 border-gray-500/30' : 'bg-orange-500 hover:bg-orange-600 text-white shadow-orange-500/20 border-orange-700 shadow-2xl'} rounded-3xl transition-all border-b-8 sm:border-b-[12px] active:border-b-0 active:translate-y-2 flex flex-col items-center justify-center gap-1 group px-4`}
                  >
                    <div className="flex flex-col items-center gap-2">
                      {isLocked ? (
                        <Lock className="w-6 h-6 sm:w-8 sm:h-8" />
                      ) : (
                        <TrendingUp className="w-6 h-6 sm:w-8 sm:h-8 transition-transform group-hover:-translate-y-2" />
                      )}
                      <span className="font-black text-base sm:text-2xl uppercase tracking-[0.2em]">
                ...
                <motion.div whileHover={{ scale: canTrade ? 1.02 : 1 }} whileTap={{ scale: canTrade ? 0.98 : 1 }}>
                  <Button
                    ...
                    disabled={disabled || !canTrade}
                    className={`w-full h-24 sm:h-32 ${isLocked ? 'bg-gray-500/20 text-gray-500 border-gray-500/30' : 'bg-blue-500 hover:bg-blue-600 text-white shadow-blue-500/20 border-blue-700 shadow-2xl'} rounded-3xl transition-all border-b-8 sm:border-b-[12px] active:border-b-0 active:translate-y-2 flex flex-col items-center justify-center gap-1 group px-4`}
                  >
                    <div className="flex flex-col items-center gap-2">
                      {isLocked ? (
                        <Lock className="w-6 h-6 sm:w-8 sm:h-8" />
                      ) : (
                        <TrendingDown className="w-6 h-6 sm:w-8 sm:h-8 transition-transform group-hover:translate-y-2" />
                      )}
                      <span className="font-black text-base sm:text-2xl uppercase tracking-[0.2em]">
