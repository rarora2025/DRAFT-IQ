...
export function Ticker() {
  ...
  const displayPlayers = [...players, ...players, ...players, ...players, ...players]

  return (
    <div className="w-full bg-background h-10 flex items-center overflow-hidden whitespace-nowrap z-[101] fixed top-0 left-0 right-0">
      <style jsx>{`
        @keyframes scroll {
        ...
        .ticker-scroll {
          display: flex;
          animation: scroll ${Math.max(players.length * 4, 30)}s linear infinite;
        }
        .ticker-scroll:hover {
          animation-play-state: paused;
        }
      `}</style>
      <div className="ticker-scroll items-center gap-10">
        {loading ? (
          ...
          displayPlayers.map((player, idx) => (
            <Link 
              ...
              className="flex items-center gap-4 group px-4 shrink-0 transition-opacity hover:opacity-80"
            >
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded-full overflow-hidden border border-white/10 bg-card flex-shrink-0 group-hover:border-primary/50 transition-colors">
                  <img 
                    src={player.pfp} 
                    alt={player.name} 
